...
<div class="mb-4">
    <div class="p-3 rounded-3 shadow-sm" style="background: linear-gradient(135deg, #e0d4ff, #ffffff);">
        <h3 class="fw-bold mb-0 text-primary">Detail Gallery</h3>
        <p class="text-muted mb-0">Informasi lengkap dari galeri</p>
    </div>
</div>
        ...
        <table class="table align-middle" style="border-collapse: separate; border-spacing: 0 10px;">
            <tbody>
                ...
                <tr class="shadow-sm">
                    ...
                    <td class="bg-white border rounded-end">{{ $gallery->title }}</td>
                </tr>

                <tr class="shadow-sm">
                    <th class="bg-light fw-semibold" style="border-radius: 8px 0 0 8px;">Description</th>
                    <td class="bg-white border rounded-end">{{ $gallery->description ?? '-' }}</td>
                </tr>
                ...
                <tr class="shadow-sm">
                    ...
                    <td class="bg-white border rounded-end">{{ $gallery->created_at }}</td>
                </tr>

                <tr class="shadow-sm">
                    <th class="bg-light fw-semibold" style="border-radius: 8px 0 0 8px;">Updated At</th>
                    <td class="bg-white border rounded-end">{{ $gallery->updated_at }}</td>
                </tr>

            </tbody>
        </table>

        <a href="{{ route('adminpanel.gallery') }}" class="btn btn-secondary mt-3">Back</a>
        <a href="{{ route('adminpanel.gallery.edit', $gallery->id) }}" class="btn btn-warning mt-3">Edit</a>
